...
async function getRegistrationData(id: string): Promise<RegistrationWithEvent | null> {
  try {
    ...
    // Générer le QR code UNIQUEMENT à partir du shortCode 
    // (prioritaire par rapport au qrCode pour le scan)
    const qrCodeDataUrl = await QRCode.toDataURL(reg.shortCode || "");
    
    return {
      ...reg,
      qrCodeUrl: qrCodeDataUrl,
      // Garantir que le shortCode est toujours défini (même si undefined) 
      // pour le composant Badge
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="text-center space-y-4 mt-8">
        <p className="text-gray-600">
          We&apos;ve also sent an email with your badge and QR code to <span className="font-semibold">{registrationData.email}</span>
        </p>
        <div className="mt-6">
          <button
